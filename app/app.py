...
@app.route("/scouting")
def scouting():
    """Scouting page — loads LLM-generated reports from the player_reports table."""
    conn = get_db_connection()

    try:
        # Load all scouting reports saved by notebook 04
        reports_df = pd.read_sql_query(
            "SELECT * FROM player_reports ORDER BY market_value_tm DESC", conn
        )
        has_data = len(reports_df) > 0
    except Exception:
        # Table doesn't exist yet (notebook 04 hasn't been run)
    ...
    if not has_data:
        # Friendly placeholder shown before the notebook has been executed
        content = """
<div class="alert alert-warning" role="alert">
  <h5 class="alert-heading">No scouting reports yet</h5>
  <p class="mb-0">
    Run <strong>notebooks/04_llm_analysis.ipynb</strong> to generate AI-powered
    scouting reports and populate this page.
    Make sure <strong>Ollama is running</strong> and <code>llama3.2</code> is pulled
    (<code>ollama pull llama3.2</code>), then run the notebook.
  </p>
</div>"""
    else:
        # Build one Bootstrap card per player
        cards_html = ""
        for _, row in reports_df.iterrows():
            ...
            age          = row.get("age",            "–")
            mv           = row.get("market_value_tm", None)
            report       = row.get("scouting_report", "No report available.")

            # Format market value
            mv_str = f"€{float(mv):.1f}M" if mv is not None and pd.notna(mv) else "–"

            # Colour-code the position badge
            badge_colours = {
                "Forward":    "danger",
                "Midfielder": "primary",
                "Defender":   "success",
            ...
            badge = badge_colours.get(position, "secondary")
    ...
    <div class="card-body">
      <div class="d-flex justify-content-between align-items-start mb-2">
        <div>
          ...
          <small class="text-muted">{nationality} · Age {age}</small>
        </div>
        <div class="text-end">
          <span class="badge bg-{badge} me-1">{position}</span>
          <span class="badge bg-light text-dark border">{mv_str}</span>
        </div>
      </div>
      <hr class="my-2">
      <p class="card-text fst-italic text-secondary" style="font-size:0.95rem">
        &#8220;{report}&#8221;
      </p>
      <small class="text-muted">Generated by llama3.2 (Ollama)</small>
    </div>
  </div>
</div>"""
...
{cards_html}
    ...
    return render_page(
        ...
        description=(
            f"llama3.2 (Ollama) generated scouting reports for the top "
            f"{len(reports_df)} players by market value."
            if has_data else
            "LLM-powered player scouting reports — run notebook 04 to generate them."
        ),
        content=content,
        active="/scouting",
    )
